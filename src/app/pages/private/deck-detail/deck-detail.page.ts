...
export class DeckDetailPage implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    this.sub = route.params.subscribe((p) => {
      this.deckService
        ...
        .get()
        .toPromise()
        .then((d) => (this.deck = d.data() as Deck));
      this.cardDbService
        .getCards(deckService.getDeckById(p.id))
        .then((cards) =>
          cards.forEach((c) => this.cards.push(c.data() as Card))
        );
    });
  }
    ...
    const buttons: ActionSheetButton[] = [
      {
        ...
        handler: async () => {
          ...
          c.count === 0
            ...
            : (this.cards[this.cards.indexOf(c)] = c);
  ...
  updateDeckByCard(deck: Deck, cards: Array<Card>) {
    const legalities = cards.map((c) =>
      c.legalities.map((l) => JSON.stringify(l))
    );
    deck.legalities = legalities
      .shift()
      .filter((v) => legalities.every((a) => a.indexOf(v) !== -1));
  ...
  public openCardDetail(card: Card) {
    this.router.navigate(['private', 'deck-detail', this.deck.id, 'card-detail', card.id]);
  }
}
